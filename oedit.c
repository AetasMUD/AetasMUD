...
static void oedit_disp_anticlass_menu(struct descriptor_data *d);
static void oedit_disp_needclass_menu(struct descriptor_data *d);
static void oedit_disp_antirace_menu(struct descriptor_data *d);
...
static void oedit_disp_anticlass_menu(struct descriptor_data *d)
{
  ...
  for (counter = 0; counter < NUM_ANTICLASS_FLAGS; counter++) {
    write_to_output(d, "%s%2d%s) %-20.20s %s", grn, counter + 1, nrm,
		anti_class_bits[counter], !(++columns % 2) ? "\r\n" : "");
  }
  sprintbitarray(GET_OBJ_ANTICLASS(OLC_OBJ(d)), anti_class_bits, EF_ARRAY_MAX, bits);
  write_to_output(d, "\r\nObject !Class Flags: %s%s%s\r\n"
	  "Enter anti-class flag (0 to quit) : ",
	  cyn, bits, nrm);
}

/* Object need class flags. */
static void oedit_disp_needclass_menu(struct descriptor_data *d)
{
  char bits[MAX_STRING_LENGTH];
  int counter, columns = 0;
  ...
  for (counter = 0; counter < NUM_NEEDCLASS_FLAGS; counter++) {
    write_to_output(d, "%s%2d%s) %-20.20s %s", grn, counter + 1, nrm,
		need_class_bits[counter], !(++columns % 2) ? "\r\n" : "");
  }
  sprintbitarray(GET_OBJ_NEEDCLASS(OLC_OBJ(d)), need_class_bits, EF_ARRAY_MAX, bits);
  write_to_output(d, "\r\nObject Class Needed Flags: %s%s%s\r\n"
	  "Enter needed-class flag (0 to quit) : ",
	  cyn, bits, nrm);
}

/* Object anti race flags. */
static void oedit_disp_antirace_menu(struct descriptor_data *d)
{
  char bits[MAX_STRING_LENGTH];
  int counter, columns = 0;
  ...
  for (counter = 0; counter < NUM_ANTIRACE_FLAGS; counter++) {
    write_to_output(d, "%s%2d%s) %-20.20s %s", grn, counter + 1, nrm,
		anti_race_bits[counter], !(++columns % 2) ? "\r\n" : "");
  }
  sprintbitarray(GET_OBJ_ANTIRACE(OLC_OBJ(d)), anti_race_bits, EF_ARRAY_MAX, bits);
  write_to_output(d, "\r\nObject !Race Flags: %s%s%s\r\n"
	  "Enter anti-race flag (0 to quit) : ",
	  cyn, bits, nrm);
}
...
static void oedit_disp_weapon_spells(struct descriptor_data *d)
{
 ...
 send_to_char(d->character, "\r\n Currently defined spell casting :\r\n\r\n");

 if (!HAS_SPELLS(OLC_OBJ(d)))
  send_to_char(d->character, "  [%s1%s] Nothing.\r\n\r\n", cyn, nrm);

 else {
 
 for (counter = 0; counter < MAX_WEAPON_SPELLS; counter++) {
  sprintf(buf, "  [%s%d%s]  Spell: %s%s%s \r\n"
	       "       Level: %s%3d%s \r\n"
	       "     Percent: %s%3d%s\r\n\r\n",
	       cyn, counter+1, nrm,
 ...
 send_to_char(d->character, " Enter spell number to edit (%s0%s to Quit) :", cyn, nrm);
}
...
static void oedit_disp_menu(struct descriptor_data *d)
{
  ...
  sprintbitarray(GET_OBJ_ANTICLASS(obj), anti_class_bits, EF_ARRAY_MAX, buf3);
  sprintbitarray(GET_OBJ_ANTIRACE(obj), anti_race_bits, EF_ARRAY_MAX, buf4);
  sprintbitarray(GET_OBJ_NEEDCLASS(obj), need_class_bits, EF_ARRAY_MAX, buf5);
  ...
  write_to_output(d,
	  ...
	  "%sU%s) Anti-Class flags: %s%s\r\n"
	  "%sV%s) Anti-Race flags: %s%s\r\n"
	  "%sN%s) Need-Class flags: %s%s\r\n",
...
void oedit_parse(struct descriptor_data *d, char *arg)
{
  ...
  case OEDIT_MAIN_MENU:
	...
	case 'u':
	case 'U':
		oedit_disp_anticlass_menu(d);
		OLC_MODE(d) = OEDIT_ANTICLASS;
		break;
		
	case 'v':
	case 'V':
		oedit_disp_antirace_menu(d);
		OLC_MODE(d) = OEDIT_ANTIRACE;
		break;
    ...
    case 'n':
	case 'N':
		oedit_disp_needclass_menu(d);
		OLC_MODE(d) = OEDIT_NEEDCLASS;
		break;	 
  ...
  case OEDIT_EXTRAS:
	...
	case OEDIT_ANTICLASS:
    number = atoi(arg);
    if ((number < 0) || (number > NUM_ANTICLASS_FLAGS)) {
      oedit_disp_anticlass_menu(d);
      return;
    } else if (number == 0)
      break;
    else {
      TOGGLE_BIT_AR(GET_OBJ_ANTICLASS(OLC_OBJ(d)), (number - 1));
      oedit_disp_anticlass_menu(d);
      return;
    }
	
	case OEDIT_NEEDCLASS:
    number = atoi(arg);
    if ((number < 0) || (number > NUM_NEEDCLASS_FLAGS)) {
      oedit_disp_needclass_menu(d);
      return;
    } else if (number == 0)
      break;
    else {
      TOGGLE_BIT_AR(GET_OBJ_NEEDCLASS(OLC_OBJ(d)), (number - 1));
      oedit_disp_needclass_menu(d);
      return;
    }
	...
	case OEDIT_ANTIRACE:
    number = atoi(arg);
    if ((number < 0) || (number > NUM_ANTIRACE_FLAGS)) {
      oedit_disp_antirace_menu(d);
      return;
    } else if (number == 0)
      break;
    else {
      TOGGLE_BIT_AR(GET_OBJ_ANTIRACE(OLC_OBJ(d)), (number - 1));
      oedit_disp_antirace_menu(d);
      return;
    }
  ...
  if((number=atoi(arg))==0)
   break;
  else if(number<0 || number>MAX_WEAPON_SPELLS) {
   oedit_disp_weapon_spells(d);
   return;
   }
   OLC_VAL(d)=number-1;
  ...
  case OEDIT_WEAPON_SPELLS:
   ...
   send_to_char(d->character, " At what level should it be cast (1 to 50) : ");
   return;
  ...
  case OEDIT_WEAPON_SPELL_LEVEL:
   number=atoi(arg);
    if(number<1 || number > 50) {
     ...
     send_to_char(d->character, " At what level should the spell be cast (1 to 50) : ");
     return;
     }
  ...
  case OEDIT_WEAPON_SPELL_PERCENT:
   ...
   if(number<0 || number>100) {
    send_to_char(d->character, " Invalid percent, must be 0-100.\r\nPlease enter the percent: ");
    return;
    }
