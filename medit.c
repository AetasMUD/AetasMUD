...
static void medit_disp_remort(struct descriptor_data *d);
...
static void init_mobile(struct char_data *mob)
{
  ...
  GET_RACE(mob) = RACE_MOBILE;
  GET_CLASS(mob) = CLASS_MOBILE;
  GET_WEIGHT(mob) = 200;
  GET_HEIGHT(mob) = 65;
...
/* Display the remort class of the mobile. */
void medit_disp_remort(struct descriptor_data *d)
{
  int i;
  char flags[MAX_STRING_LENGTH];
...
static void medit_disp_menu(struct descriptor_data *d)
{
  ...
  sprinttype(GET_RACE(mob), race_types, flags, sizeof(flags));
  sprinttype(GET_CLASS(mob), class_types, flag2, sizeof(flag2));
  ...
  sprinttype(GET_REMORT(mob), class_types, flags, sizeof(flags));
  write_to_output(d,
  "%sU%s) Remort    : %s%s\r\n",
     grn, nrm, yel, flags
	 );
...
static void medit_disp_stats_menu(struct descriptor_data *d)
{
  ...
  "(%sC%s) Gold:        %s[%s%10d%s]%s\r\n\r\n",
	  ...
	  cyn, nrm, cyn, yel, GET_HEIGHT(mob), cyn, nrm,  cyn, nrm, cyn, yel, GET_WEIGHT(mob), cyn, nrm,
...
/* Numerical responses. */
  ...
  case MEDIT_RACE:
    GET_RACE(OLC_MOB(d)) = LIMIT(i, 0, NUM_RACES -1);
    break;
  ...
  case MEDIT_GEN:
    GET_GEN(OLC_MOB(d)) = LIMIT(i, 0, 10);
    OLC_VAL(d) = TRUE;
    medit_disp_stats_menu(d);
    return;
	
  case MEDIT_HEIGHT:
    GET_HEIGHT(OLC_MOB(d)) = LIMIT(i, 0, 5000);
    OLC_VAL(d) = TRUE;
    medit_disp_stats_menu(d);
    return;
	
  case MEDIT_WEIGHT:
    GET_WEIGHT(OLC_MOB(d)) = LIMIT(i, 0, 5000);
    OLC_VAL(d) = TRUE;
    medit_disp_stats_menu(d);
    return;
