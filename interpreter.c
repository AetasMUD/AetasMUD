...
cpp_extern const struct command_info cmd_info[] = {
  ...
  { "anonymous", "anon"    , POS_DEAD    , do_gen_tog  , 0, SCMD_ANON },
  ...
  { "assign"   , "assign"  , POS_RESTING , do_assign   , LVL_IMPL, 0 }, 
  ...
  { "badge"    , "badge"   , POS_DEAD    , do_badge    , LVL_GRGOD, 0 },
  ...
  { "clan"     , "clan"	   , POS_SLEEPING, do_clan     , 0, 0 },
  { "csay"     , "cs"	   , POS_SLEEPING, do_clan_say , 0, 0 },
  ...
  { "experience","exp"	   , POS_SLEEPING, do_experience, 0, 0 },
  ...
  { "noaff"    , "noaff"   , POS_DEAD    , do_gen_tog  , 0, SCMD_NO_AFF },
  ...
  { "nowho"    , "nowho"   , POS_DEAD    , do_gen_tog  , LVL_IMMORT, SCMD_NOWHO },
  ...
  { "skills"   , "skills"  , POS_SLEEPING, do_skills   , 0, 0 },
  ...
  { "spells"   , "spells"  , POS_SLEEPING, do_spells   , 0, 0 },
  ...
  { "worldwrite", "worldw" , POS_DEAD    , do_gen_tog  , LVL_GRGOD, SCMD_WRLDWRITE },
 ...
 *  * uses strcasecmp.     by ROD 09/04/1999
 *   */
...
char *add_double_atsign(char *string)
{
  char *ddread, *ddwrite;
...
void nanny(struct descriptor_data *d, char *arg)
{
  ...
  case CON_CHPWD_VRFY:
    ...
    echo_on(d);
	SET_BIT_AR(PRF_FLAGS(d->character), PRF_CLS);

    if (STATE(d) == CON_CNFPASSWD) {
	  clear_screen(d);
	  write_to_output(d, "\r\nWould you like to turn on color (Y/N)? ");
	  STATE(d) = CON_QCOLOR;
	  } else {
      save_char(d->character);
      write_to_output(d, "\r\nDone.\r\n%s", CONFIG_MENU);
  ...
  case CON_QCOLOR:
    switch (*arg) 
	{
      ...
      case 'N':
        ...
        break;
      default:
        write_to_output(d, "That is not a proper response. Use y or n.");
       return;
    }
  ...
  /**********************  RACE Selection *************************************/
  // Depending on class, some races aren't available 
  
  // Mage races
  if (GET_CLASS(d->character) == CLASS_MAGE)
  {
    clear_screen(d);
    ...
    case CON_QMAGERACE:
      load_result = parse_mage_race(arg);
  ...
  if (GET_CLASS(d->character) == CLASS_NECROMANCER)
  {
    ...
    case CON_QNECRORACE:
      load_result = parse_necro_race(arg);
  ...
  if (GET_CLASS(d->character) == CLASS_MONK)
  {
    ...
    case CON_QMONKRACE:
      load_result = parse_monk_race(arg);
  ...
  if (GET_CLASS(d->character) == CLASS_KNIGHT)
  {
    ...
    case CON_QKNIGRACE:
      load_result = parse_knig_race(arg);
  ...
  if (GET_CLASS(d->character) == CLASS_CLERIC)
  {
    ...
    case CON_QCLERRACE:
      load_result = parse_cler_race(arg);
  ...
  // Barbarian races
  if (GET_CLASS(d->character) == CLASS_BARBARIAN)
  {
    clear_screen(d);
    ...
    case CON_QBARBRACE:
      load_result = parse_barb_race(arg);
  ...
  if (GET_CLASS(d->character) == CLASS_DRUID)
  {
    ...
    case CON_QDRUIDRACE:
      load_result = parse_druid_race(arg);
  ...
  if (GET_CLASS(d->character) == CLASS_PSIONICIST)
  {
    ...
    case CON_QPSIORACE:
      load_result = parse_psio_race(arg);
  ...
  // The classes without racial restrictions fall to here
  else
    clear_screen(d);
  ...
  case CON_QRACE:
    load_result = parse_race_by_abbrev(arg);
  ...
  // Set the character's remort class to its primary class, just to have it as something
  GET_REMORT(d->character) = GET_CLASS(d->character);
