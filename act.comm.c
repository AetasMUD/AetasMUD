...
ACMD(do_say_adv)
{
  ...
  if (!*buf || !*buf2) {
    sprintf(buf, "Whom do you want to talk to, and what do you want to say?\r\n");
...
static void perform_tell(struct char_data *ch, struct char_data *vict, char *arg)
{
  ...
  snprintf(buf, sizeof(buf), "%s$n tells you, %s'%s'%s", CCRED(vict, C_NRM), CCNRM(ch, C_NRM), arg, CCNRM(vict, C_NRM));
  ...
  if (!IS_NPC(ch) && PRF_FLAGGED(ch, PRF_NOREPEAT))
    ...
    snprintf(buf, sizeof(buf), "%sYou tell $N, %s'%s'%s", CCRED(ch, C_NRM), CCNRM(ch, C_NRM), arg, CCNRM(ch, C_NRM));
    msg = act(buf, FALSE, ch, 0, vict, TO_CHAR | TO_SLEEP);     
    add_history(ch, msg, HIST_TELL);
	
	if (PRF_FLAGGED(vict, PRF_AFK))
    act("$E is AFK right now; try again later.", FALSE, ch, 0, vict, TO_CHAR);
  }
