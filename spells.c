...
ASPELL(spell_identify)
{
  ...
  if (obj) {
	...
	sprintbitarray(GET_OBJ_ANTICLASS(obj), anti_class_bits, EF_ARRAY_MAX, bitbuf);
    send_to_char(ch, "Item is: %s\r\n", bitbuf);
	
	sprintbitarray(GET_OBJ_ANTIRACE(obj), anti_race_bits, EF_ARRAY_MAX, bitbuf);
    send_to_char(ch, "Item is: %s\r\n", bitbuf);
    ...
    case ITEM_WEAPON:
	  ...
	  if (HAS_SPELLS(obj)) {
       send_to_char(ch, "Casts: %s at level %d\r\n",
              skill_name(GET_WEAPON_SPELL(obj, 0)), GET_WEAPON_SPELL_LVL(obj, 0));
     }
    ...
    if (!IS_NPC(victim)) {
	  send_to_char(ch, "%s is %d years, %d months, %d days and %d hours old.\r\n", GET_NAME(victim), age(victim)->year, age(victim)->month, age(victim)->day, age(victim)->hours);
	}
	
	sprinttype(GET_RACE(victim), race_types, bitbuf, sizeof(bitbuf));
	send_to_char(ch, "Race: %s, ", bitbuf);
	sprinttype(GET_CLASS(victim), class_types, bitbuf, sizeof(bitbuf));
	send_to_char(ch, "Class: %s, Alignment: %d\r\n", bitbuf, GET_ALIGNMENT(victim));

    send_to_char(ch, "Height %d inches, Weight %d pounds\r\n", GET_HEIGHT(victim), GET_WEIGHT(victim));
    send_to_char(ch, "Level: %d, Hits: %d, Mana: %d, Move: %d\r\n", GET_LEVEL(victim), GET_HIT(victim), GET_MANA(victim), GET_MOVE(victim));
