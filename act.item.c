...
static void wear_message(struct char_data *ch, struct obj_data *obj, int where)
{
  const char *wear_messages[][2] = {
	...
	{OBJ_FLAGGED(obj, ITEM_TWO_HANDED) ?
	    "$n wields $p with both hands." : "$n wields $p.",
    OBJ_FLAGGED(obj, ITEM_TWO_HANDED) ? 
	    "You wield $p with both hands." : "You wield $p."},
...
static void perform_wear(struct char_data *ch, struct obj_data *obj, int where)
{
 char buf[MAX_STRING_LENGTH];
  ...
  /* first check to make sure the object isn't restricted by class or race */
  
  if (invalid_class(ch, obj)) {
	send_to_char(ch, "Your class is restricted from using %s.\r\n", 
	obj->short_description);
    return;
  }
  
  if (invalid_needed_class(ch, obj)) {
	send_to_char(ch, "You are not the appropriate class to use %s.\r\n", 
	obj->short_description);
    return;
  }
  
  if (invalid_race(ch, obj)) {
    sprinttype(ch->player.chrace, raceplural_types, buf, sizeof(buf));
	send_to_char(ch, "Your race is restricted from using %s.\r\n", 
	obj->short_description);
    return;
   }
  ...
  if (OBJ_FLAGGED(obj, ITEM_TWO_HANDED)) 
   if ((GET_EQ(ch, WEAR_HOLD)) || (GET_EQ(ch, WEAR_SHIELD))) { 
	send_to_char(ch, "You need both hands free to wield %s.\r\n", obj->short_description);
    return;
   }
   
  if ((where == WEAR_WIELD) || (where == WEAR_SHIELD) || (where == WEAR_HOLD))
   if ((GET_EQ(ch, WEAR_WIELD)) 
     && (OBJ_FLAGGED(GET_EQ(ch, WEAR_WIELD), ITEM_TWO_HANDED))) {
    	send_to_char(ch, "You need a free hand to use %s.\r\n", obj->short_description);
    return;
   }
   
   if ((where == WEAR_2WIELD))
    if ((GET_EQ(ch, WEAR_WIELD))) {
    	send_to_char(ch, "You aren't skilled enough to use two weapons.\r\n");
    return;
   }
...
ACMD(do_sac)
{
  ...
  switch (rand_number(0, 2)) {
      case 0:
        ...
        GET_MANA(ch) = MIN(GET_MAX_MANA(ch), GET_MANA(ch) + rand_number(1, 10));
      break;
      case 1:
        send_to_char(ch, "You sacrifice %s to the Gods.\r\n", GET_OBJ_SHORT(j));
        GET_MOVE(ch) = MIN(GET_MAX_MOVE(ch), GET_MOVE(ch) + rand_number(1, 10));
      break;
      case 2:
        send_to_char(ch, "You sacrifice %s to the Gods.\r\n", GET_OBJ_SHORT(j));
        GET_HIT(ch) = MIN(GET_MAX_HIT(ch), GET_HIT(ch) + rand_number(1, 10));
      break;
      default:
