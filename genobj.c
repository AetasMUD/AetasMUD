...
int save_objects(zone_rnum zone_num)
{
  ...
  char cbuf1[MAX_STRING_LENGTH], cbuf2[MAX_STRING_LENGTH], cbuf3[MAX_STRING_LENGTH], cbuf4[MAX_STRING_LENGTH];
  char nbuf1[MAX_STRING_LENGTH], nbuf2[MAX_STRING_LENGTH];
  ...
  for (counter = genolc_zone_bottom(zone_num); counter <= zone_table[zone_num].top; counter++) {
	...
	*buf = '\0';
	  ...
	  sprintascii(cbuf1, GET_OBJ_ANTICLASS(obj)[0]);
      sprintascii(cbuf2, GET_OBJ_ANTICLASS(obj)[1]);
      sprintascii(cbuf3, GET_OBJ_ANTIRACE(obj)[0]);
      sprintascii(cbuf4, GET_OBJ_ANTIRACE(obj)[1]);
	  sprintascii(nbuf1, GET_OBJ_NEEDCLASS(obj)[0]);
      sprintascii(nbuf2, GET_OBJ_NEEDCLASS(obj)[1]);

      fprintf(fp, "%d %s %s %s %s %s %s %s %s %s %s %s %s %s %s %s %s %s %s\n"
          "%d %d %d %d\n"
          "%d %d %d %d %d\n",
