...
static void prefedit_disp_main_menu(struct descriptor_data *d)
{
  ...
  sprintf(prompt_string, "%s%s%s%s", PREFEDIT_FLAGGED(PRF_DISPHP) ? "H" : "",   PREFEDIT_FLAGGED(PRF_DISPMANA) ? "M" : "",
                                   PREFEDIT_FLAGGED(PRF_DISPMOVE) ? "V" : "", PREFEDIT_FLAGGED(PRF_DISPALIGN) ? "A" : "" );
  ...
  send_to_char(d->character, "\r\n"
                             "%sPreferences\r\n"
                             "%sP%s) Prompt : %s[%s%-4s%s]         %sL%s) Pagelength : %s[%s%-3d%s]\r\n"
...
/* Line 10 - autodiagnose */			 
			 CBYEL(d->character, C_NRM), CCNRM(d->character, C_NRM), CCCYN(d->character, C_NRM), PREFEDIT_FLAGGED(PRF_AUTODIAG) ? CBGRN(d->character, C_NRM) : CBRED(d->character, C_NRM),
             ONOFF(PREFEDIT_FLAGGED(PRF_AUTODIAG)), CCCYN(d->character, C_NRM)
             );
  ...
  send_to_char(d->character, "\r\n"
							 ...
							 "%sK%s) No Affects   %s[%s%3s%s]      %sL%s) Anonymous%s[%s%3s%s]%s\r\n",
             CBWHT(d->character, C_NRM),
...
/* Line 12 - NoAffects */
			 CBYEL(d->character, C_NRM), CCNRM(d->character, C_NRM), CCCYN(d->character, C_NRM), CCYEL(d->character, C_NRM),
             ONOFF(PREFEDIT_FLAGGED(PRF_NO_AFF)), CCCYN(d->character, C_NRM),
/* Line 12 - Anonymous */
			 CBYEL(d->character, C_NRM), CCNRM(d->character, C_NRM), CCCYN(d->character, C_NRM), CCYEL(d->character, C_NRM),
...
void prefedit_parse(struct descriptor_data * d, char *arg)
{
  ...
  case PREFEDIT_MAIN_MENU:
    ...
    case 'w':
    case 'W':
      send_to_char(d->character, "Enter HP at which to flee (0-%d): ", GET_MAX_HIT(d->character)/2);
  ...
  case PREFEDIT_WIMPY:
    number = atoi(arg);
    OLC_PREFS(d)->wimp_level = MAX(0, number);
    break;
  ...
  case PREFEDIT_TOGGLE_MENU:
    switch (*arg) {
	  ...
	  case 'j':
	  case 'J':
        TOGGLE_BIT_AR(PREFEDIT_GET_FLAGS, PRF_AUTODIAG);
        break;	
	  
	  case 'k':
	  case 'K':
        TOGGLE_BIT_AR(PREFEDIT_GET_FLAGS, PRF_NO_AFF);
        break;
  ...
  case PREFEDIT_PROMPT:
    ...
    if ((number < 0) || (number > 7)) {
      ...
      else
      {
        ...
        {
		 REMOVE_BIT_AR(PREFEDIT_GET_FLAGS, PRF_DISPAUTO);
          if (PREFEDIT_FLAGGED(PRF_DISPALIGN))
            REMOVE_BIT_AR(PREFEDIT_GET_FLAGS, PRF_DISPALIGN);
          else
            SET_BIT_AR(PREFEDIT_GET_FLAGS, PRF_DISPALIGN);
        }
        else if (number == 5)
        {
          if (PREFEDIT_FLAGGED(PRF_DISPAUTO))
...
void prefedit_Restore_Defaults(struct descriptor_data *d)
{
  ...
  /* PRF_AUTODIAG   - Off */
  if (PREFEDIT_FLAGGED(PRF_AUTODIAG))
     REMOVE_BIT_AR(PREFEDIT_GET_FLAGS, PRF_AUTODIAG);
	 
  /* PRF_NO_AFF   - Off */
  if (PREFEDIT_FLAGGED(PRF_NO_AFF))
     REMOVE_BIT_AR(PREFEDIT_GET_FLAGS, PRF_NO_AFF);
	 
  /* PRF_ANON   - Off */
  if (PREFEDIT_FLAGGED(PRF_ANON))
     REMOVE_BIT_AR(PREFEDIT_GET_FLAGS, PRF_ANON);
