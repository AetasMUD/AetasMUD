...
static char *make_prompt(struct descriptor_data *d)
{
  ...
  if (d->showstr_count)
    snprintf(prompt, sizeof(prompt),
      "%s[ Return to continue, (%sq%s)uit, (%sr%s)efresh, (%sb%s)ack, or page number (%s%d/%d%s) ]%s",
      CBBLK(d->character, C_NRM), CBWHT(d->character, C_NRM), CBBLK(d->character, C_NRM), 
	  CBWHT(d->character, C_NRM), CBBLK(d->character, C_NRM), CBWHT(d->character, C_NRM), 
	  CBBLK(d->character, C_NRM), CBWHT(d->character, C_NRM), d->showstr_page, 
	  d->showstr_count, CBBLK(d->character, C_NRM), CCNRM(d->character, C_NRM));
  else if (d->str)
    ...
    *prompt = '\0';
    ...
    if (GET_INVIS_LEV(d->character) && len < sizeof(prompt)) {
      count = snprintf(prompt + len, sizeof(prompt) - len, "%s(%si%d%s)%s ", 
	  CCBLU(d->character, C_NRM), CCMAG(d->character, C_NRM), GET_INVIS_LEV(d->character), 
    ...
    if (PRF_FLAGGED(d->character, PRF_DISPAUTO) && len < sizeof(prompt)) {
      ...
      if (GET_HIT(ch) << 2 < GET_MAX_HIT(ch) ) {
        count = snprintf(prompt + len, sizeof(prompt) - len, "%s%d%sH%s ", 
		CBGRN(d->character, C_NRM), GET_HIT(d->character),
		CBYEL(d->character, C_NRM), CCNRM(d->character, C_NRM));
        if (count >= 0)
          len += count;
      }
      if (GET_MANA(ch) << 2 < GET_MAX_MANA(ch) && len < sizeof(prompt)) {
        count = snprintf(prompt + len, sizeof(prompt) - len, "%s%d%sM%s ", 
		CBMAG(d->character, C_NRM), GET_MANA(d->character),
		CBYEL(d->character, C_NRM), CCNRM(d->character, C_NRM));
        if (count >= 0)
          len += count;
      }
      if (GET_MOVE(ch) << 2 < GET_MAX_MOVE(ch) && len < sizeof(prompt)) {
        count = snprintf(prompt + len, sizeof(prompt) - len, "%s%d%sV%s ", 
		CBCYN(d->character, C_NRM), GET_MOVE(d->character),
		CBYEL(d->character, C_NRM), CCNRM(d->character, C_NRM));
        if (count >= 0)
          len += count;
      }
	  ...
	  if (PRF_FLAGGED(d->character, PRF_DISPALIGN) && len < sizeof(prompt)) {
        count = snprintf(prompt + len, sizeof(prompt) - len, "%s%d%sA%s ", 
		(IS_EVIL(d->character) ? CBRED(d->character, C_NRM) :  IS_GOOD(d->character) ? CBBLU(d->character, C_NRM) : CBWHT(d->character, C_NRM)), 
		GET_ALIGNMENT(d->character), CBYEL(d->character, C_NRM), CCNRM(d->character, C_SPR));
        if (count >= 0)
          len += count;
      }
    }
    ...
    if (PRF_FLAGGED(d->character, PRF_BUILDWALK) && len < sizeof(prompt)) {
      count = snprintf(prompt + len, sizeof(prompt) - len, "%s[BUILDWALKING]%s ",
    ...
    if (PRF_FLAGGED(d->character, PRF_AFK) && len < sizeof(prompt)) {
      count = snprintf(prompt + len, sizeof(prompt) - len, "%s-AFK-%s ",
     ...
     { 
       count = snprintf(prompt + len, sizeof(prompt) - len, "%s(motd)%s ",
 ...
 * this descriptor is in the read set).  JE 2/23/95. */
  ...
  while (nl_pos != NULL) {
    ...
    /* The '> 1' reserves room for a '$ => $$' or '@' => '@@' expansion. */
    for (ptr = read_point; (space_left > 1) && (ptr < nl_pos); ptr++) {
      if (*ptr == '\b' || *ptr == 127) { /* handle backspacing or delete key */
	if (write_point > tmp) {
	  if (*(--write_point) == '$' || (*write_point == '@' && t->character && GET_LEVEL(t->character) < LVL_IMPL && !IS_NPC(t->character))) {
	...
	if ((*(write_point++) = *ptr) == '$') {		/* copy one character */
	  *(write_point++) = '$';	/* if it's a $, double it */
	  space_left -= 2;
	} else if (*ptr == '@' && t->character && GET_LEVEL(t->character) < LVL_IMPL && !IS_NPC(t->character)) {
         *(write_point++) = '@';       /* if it's a @, double it for non-implementors */
        space_left -= 2;
    }
	else
	  space_left--;
