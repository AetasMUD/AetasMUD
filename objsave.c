...
int objsave_save_obj_record(struct obj_data *obj, FILE *fp, int locate)
{
  ...
  if (GET_OBJ_ANTICLASS(obj) != GET_OBJ_ANTICLASS(temp)) 
    fprintf(fp, "CFlag: %d %d\n", GET_OBJ_ANTICLASS(obj)[0], GET_OBJ_ANTICLASS(obj)[1]);
	
  if (GET_OBJ_ANTIRACE(obj) != GET_OBJ_ANTIRACE(temp)) 
    fprintf(fp, "RFlag: %d %d\n", GET_OBJ_ANTIRACE(obj)[0], GET_OBJ_ANTIRACE(obj)[1]);
	
  if (GET_OBJ_NEEDCLASS(obj) != GET_OBJ_NEEDCLASS(temp)) 
    fprintf(fp, "NFlag: %d %d\n", GET_OBJ_NEEDCLASS(obj)[0], GET_OBJ_NEEDCLASS(obj)[1]);
  ...
  if (location > 0) {  /* Was wearing it. */
    ...
    case WEAR_HOLD:
      if (CAN_WEAR(obj, ITEM_WEAR_HOLD))
        break;
      if (IS_BARBARIAN(ch) && CAN_WEAR(obj, ITEM_WEAR_WIELD) && GET_OBJ_TYPE(obj) == ITEM_WEAPON)
        break;
      location = LOC_INVENTORY;
      break;
    default:
...
obj_save_data *objsave_parse_objects(FILE *fl)
{
  ...
  while (TRUE) {
    ...
    case 'C':
	  ...
	  } else if (!strcmp(tag, "CFlag")) {
        sscanf(line, "%s %s", f1, f2);
        GET_OBJ_ANTICLASS(temp)[0] = asciiflag_conv(f1);
        GET_OBJ_ANTICLASS(temp)[1] = asciiflag_conv(f2);
      }
      break;
    case 'D':
      if (!strcmp(tag, "Desc"))
    ...
    case 'N':
	  ...
	  } else if (!strcmp(tag, "NFlag")) {
        sscanf(line, "%s %s", f1, f2);
        GET_OBJ_NEEDCLASS(temp)[0] = asciiflag_conv(f1);
        GET_OBJ_NEEDCLASS(temp)[1] = asciiflag_conv(f2);
      }
      break;
    case 'P':
      if (!strcmp(tag, "Perm")) {
	...
	} else if (!strcmp(tag, "RFlag")) {
        sscanf(line, "%s %s", f1, f2);
        GET_OBJ_ANTIRACE(temp)[0] = asciiflag_conv(f1);
        GET_OBJ_ANTIRACE(temp)[1] = asciiflag_conv(f2);
      }
      break;
    case 'S':
      if (!strcmp(tag, "Shrt"))
