...
void gain_exp(struct char_data *ch, int gain)
{
  ...
  if (!IS_NPC(ch) && ((GET_LEVEL(ch) < 1 || GET_LEVEL(ch) >= 50)))
    return;
...
char *center_text(char *txt, int len)
{
  ...
  if (len <= curlen) return txt;
  sprintf(tmp, "%-*s", ((len-curlen)/2)+curlen+odd, txt);
  sprintf(buf, "%*s", len, tmp);
  
  return buf;
}

/* update blood in rooms */
void update_blood(void)
{
 int i;
