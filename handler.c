...
{
  if (OBJ_ANTICLASS_FLAGGED(obj, ITEM_ANTI_EVIL) && IS_EVIL(ch))
    return TRUE;
  if (OBJ_ANTICLASS_FLAGGED(obj, ITEM_ANTI_GOOD) && IS_GOOD(ch))
    return TRUE;
  if (OBJ_ANTICLASS_FLAGGED(obj, ITEM_ANTI_NEUTRAL) && IS_NEUTRAL(ch))
    return TRUE;
  return FALSE;
}
...
void equip_char(struct char_data *ch, struct obj_data *obj, int pos)
{
  ...
  if (invalid_class(ch, obj)) {
    act("Members of your class are unable to use $p.", FALSE, ch, obj, 0, TO_CHAR);
	act("You stop using $p.", FALSE, ch, obj, 0, TO_CHAR);
	act("$n stops using $p.", FALSE, ch, obj, 0, TO_ROOM);
    /* Changed to drop in inventory instead of the ground. */
    obj_to_char(obj, ch);
    return;
  }
  ...
  if (invalid_race(ch, obj)) {
    act("Members of your race are unable to use $p.", FALSE, ch, obj, 0, TO_CHAR);
	act("You stop using $p.", FALSE, ch, obj, 0, TO_CHAR);
	act("$n stops using $p.", FALSE, ch, obj, 0, TO_ROOM);
    /* Changed to drop in inventory instead of the ground. */
    obj_to_char(obj, ch);
