...
const char *trade_letters[] = {
        ...
        "Evil",
        "Neutral",
        "Knight",           /* Then the class based ones */
        "Cleric",
        "Barbarian",
        "Ranger",
        "Mage",
...
static int is_ok_char(struct char_data *keeper, struct char_data *ch, int shop_nr)
{
  ...
  if ((IS_KNIGHT(ch) && NOTRADE_KNIGHT(shop_nr)) ||
      ...
      (IS_PSIONICIST(ch) && NOTRADE_PSIONICIST(shop_nr))) { 
...
bool shopping_identify(char *arg, struct char_data *ch, struct char_data *keeper, int shop_nr)
{
  ...
  if (!*arg) {
    snprintf(bitbuf, sizeof(bitbuf), "%s What do you want to identify??", GET_NAME(ch));
    do_tell(keeper, bitbuf, cmd_tell, 0);
    return TRUE;
  }
  if (!(obj = get_purchase_obj(ch, arg, keeper, shop_nr, TRUE)))
    return FALSE;
