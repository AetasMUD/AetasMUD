...
static void do_stat_object(struct char_data *ch, struct obj_data *j)
{
  ...
  sprintbitarray(GET_OBJ_ANTICLASS(j), anti_class_bits, EF_ARRAY_MAX, buf);
  send_to_char(ch, "!Class flags  : %s\r\n", buf);
  
  sprintbitarray(GET_OBJ_ANTIRACE(j), anti_race_bits, EF_ARRAY_MAX, buf);
  send_to_char(ch, "!Race flags   : %s\r\n", buf);
  
  sprintbitarray(GET_OBJ_NEEDCLASS(j), need_class_bits, EF_ARRAY_MAX, buf);
  send_to_char(ch, "Class flags  : %s\r\n", buf);
  ...
  case ITEM_WEAPON:
   if (HAS_SPELLS(j)) {
    send_to_char(ch, "Todam: %dd%d, Avg Damage: %.1f. Message type: %s\r\n"
					 "Casts: %s at level %d, after %d percent of rounds\r\n",
	    GET_OBJ_VAL(j, 1), GET_OBJ_VAL(j, 2), ((GET_OBJ_VAL(j, 2) + 1) / 2.0) * GET_OBJ_VAL(j, 1),  
		attack_hit_text[GET_OBJ_VAL(j, 3)].singular,
	    skill_name(GET_WEAPON_SPELL(j, 0)), GET_WEAPON_SPELL_LVL(j, 0),
	    GET_WEAPON_SPELL_PCT(j, 0));
    break;
   }
   else {
    send_to_char(ch, "Todam: %dd%d, Avg Damage: %.1f. Message type: %s\r\n"
...
static void do_stat_character(struct char_data *ch, struct char_data *k)
{
  ...
  if (!IS_MOB(k))
	...
	sprinttype(k->player.chrace, race_types, buf2, sizeof(buf2));
	send_to_char(ch, "Class: %s, Race: %s, Lev: [%s%2d%s], XP: [%s%7d%s], Align: [%4d]\r\n",
		buf, buf2, CCYEL(ch, C_NRM), GET_LEVEL(k), CCNRM(ch, C_NRM),
		CCYEL(ch, C_NRM), GET_EXP(k), CCNRM(ch, C_NRM), GET_ALIGNMENT(k));
	sprinttype(k->player.remort_class, class_types, buf, sizeof(buf));
	send_to_char(ch, "Remort: %s, Generation: [%s%d%s]\r\n", 
		buf, CCYEL(ch, C_NRM), GET_GEN(k), CCNRM(ch, C_NRM));
	
  if (!IS_NPC(k)) {
	send_to_char(ch, "Clan: [%d][%s], Rank: [%d][%s]\r\n", GET_CLAN(k),
    get_clan_name(GET_CLAN(k)), GET_CLAN_RANK(k), get_rank_name(GET_CLAN(k), GET_CLAN_RANK(k)));
	}

  if (!IS_NPC(k)) {
    char buf1[64], buf2[64];

	/* changed this to a format that shows the year as well
	...
	*/
    ...
    if (GET_LEVEL(k) >= LVL_BUILDER) {
	  send_to_char(ch, "Badge: %s", GET_BADGE(k) ? GET_BADGE(k) : "<None>");
...
ACMD(do_show)
{
  int i, j, k, l, con, c_number, builder =0;		/* i, j, k to specifics? */
  size_t len, nlen;
  zone_rnum zrn;
  zone_vnum zvn;
  byte self = FALSE;
  ...
  case 11:
   send_to_char(ch, "Current clans of Aetas:\r\n-----------------------\r\n");
  ...
  case 12:
    ...
    if (!*value) {
      send_to_char(ch, "A clan number would help.\r\n");
      return;
    }

    if (c_number > NUM_CLANS || c_number < 1)
      send_to_char(ch, "That clan does not exist.");
    else {
      send_to_char(ch, "Members of clan %s: \r\n", get_clan_name(c_number));
      for (i = 0; i <= top_of_p_table; i++ ) {
        if ((player_table + i)->clan_name ==  c_number) {
          send_to_char(ch, "%s  [%s%s%s] %-15.15s", buf, 
            ((player_table + i)->clan_rank == 10 ?
            CCCYN(ch, C_NRM) : CCNRM(ch, C_NRM)), 
            get_rank_name((player_table + i)->clan_name,
            (player_table + i)->clan_rank), CCNRM(ch, C_NRM), 
            (player_table + i)->name);
          k++;
            if (k == 3) {
              k = 0;
  ...
  /* show thaco */
  case 13:
    len = strlcpy(buf, "LvL - Kn Cl Ba Ra Ma Mo Ne Dr Th Ps\r\n----------------\r\n", sizeof(buf));
			
    for (j = 1; j < LVL_IMMORT; j++) {
      nlen = snprintf(buf + len, sizeof(buf) - len,  "%-3d - %-2d %-2d %-2d %-2d %-2d %-2d %-2d %-2d %-2d %-2d\r\n", j, 
				thaco(CLASS_KNIGHT, j),
				thaco(CLASS_CLERIC, j),
                thaco(CLASS_BARBARIAN, j),
                thaco(CLASS_RANGER, j),
                thaco(CLASS_MAGE, j),
                thaco(CLASS_MONK, j),
                thaco(CLASS_NECROMANCER, j),
                thaco(CLASS_DRUID, j),
                thaco(CLASS_THIEF, j),
				thaco(CLASS_PSIONICIST, j));
      if (len + nlen >= sizeof(buf))
        break;
      len += nlen;		
		}
  ...
  case 14:
	...
	for (i = 1; i < LVL_IMMORT; i++) { 		
	  nlen = snprintf(buf + len, sizeof(buf) - len,  "%-3d - %-6d %-6d %-6d %-6d %-6d %-6d %-6d %-6d %-6d %-6d\r\n", i,  
				level_exp(CLASS_KNIGHT, i) - level_exp(CLASS_KNIGHT, i - 1),
                ...
                level_exp(CLASS_MAGE, i) - level_exp(CLASS_MAGE, i - 1),
                level_exp(CLASS_MONK, i) - level_exp(CLASS_MONK, i - 1),
                level_exp(CLASS_NECROMANCER, i) - level_exp(CLASS_NECROMANCER, i - 1),
                level_exp(CLASS_DRUID, i) - level_exp(CLASS_DRUID, i - 1),
                level_exp(CLASS_THIEF, i) - level_exp(CLASS_THIEF, i - 1),
				level_exp(CLASS_PSIONICIST, i) - level_exp(CLASS_PSIONICIST, i - 1));
	  if (len + nlen >= sizeof(buf))
		break;
	  len += nlen;		
    }
...
/* The set options available */
  struct set_struct {
   ...
   { "race",		LVL_BUILDER, 	PC, 	MISC },
   { "remort",		LVL_BUILDER, 	PC, 	MISC },
   { "generation",		LVL_BUILDER, 	PC, 	NUMBER },  /* 60 */
   { "soulpoints",		LVL_BUILDER, 	PC, 	NUMBER },
   { "nowho",		LVL_GRGOD, PC, BINARY },
   { "badge",		LVL_GRGOD, 	PC, 	MISC   },
   { "\n", 0, BOTH, MISC }
  };
...
static int perform_set(struct char_data *ch, struct char_data *vict, int mode, char *val_arg)
{
  ...
  switch (mode) {
    ...
    case 6:  /* cha */
      ...
      break;
    case 7: /* class */
      if ((i = parse_class_by_abbrev(val_arg)) == CLASS_UNDEFINED) {
        send_to_char(ch, "That is not a player class.\r\n");
        return (0);
      }
      GET_CLASS(vict) = i;
      break;
   ...
   case 21: /* int */
    ...
    case 57: /* questhistory */
     ...
     qvnum, GET_NAME(vict));
        ...
        break;
	case 58: /* race */
      if ((i = parse_race_by_abbrev(val_arg)) == RACE_UNDEFINED) {
        send_to_char(ch, "That is not a player race.\r\n");
        return (0);
      }
      GET_RACE(vict) = i;
      break;
      }
	case 59: /* remort class */
      if ((i = parse_class_by_abbrev(val_arg)) == CLASS_UNDEFINED) {
        send_to_char(ch, "That is not a player class.\r\n");
        return (0);
      }
      ...
      break;
	case 60: /* generation */
	if (value < 0 || value > 10) {	/* Arbitrary limits. */
        send_to_char(ch, "Gen 0 to 10 accepted.\r\n");
        return (0);
      }
	...
	case 62: /* nowho */
	  ...
	  break;
	case 63: /* badge */
	 if (strlen(val_arg) > MAX_BADGE_LENGTH) {
	...
	if (!*val_arg)
          ...
          GET_BADGE(vict) = strdup(val_arg);
		  send_to_char(ch, "%s's badge is now: %s\r\n", GET_NAME(vict), GET_BADGE(vict));
        }
      break;
    default:
...
/* Armor class limits*/
#define TOTAL_WEAR_CHECKS  (NUM_ITEM_WEARS-3)  /*minus Wield, 2Wield and Take*/
struct zcheck_armor {
