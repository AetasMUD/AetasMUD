...
void sort_spells(void)
{
  ...
  qsort(&spell_sort_info[1], MAX_SPELLS, sizeof(int), compare_spells);
}
...
void sort_skills(void)
{
  ...
  /* initialize array, avoiding reserved. */
  for (a = MAX_SPELLS; a <= MAX_SKILLS; a++)
    skill_sort_info[a] = a;
...
SPECIAL(guildmaster)
{
  ...
  /* If the player is not the same class or remort as the guildmaster's prime class, don't train */
  
  if (GET_CLASS(self) != GET_CLASS(ch) && GET_CLASS(self) != GET_REMORT(ch)){
   snprintf(buf, sizeof(buf), "%s$n tells you, %s'You need to seek out a guildmaster of your class.'%s", \
   CCRED(ch, C_NRM), CCNRM(ch, C_NRM), CCNRM(ch, C_NRM));
   msg = act(buf, FALSE, self, 0, ch, TO_VICT | TO_SLEEP);
   return (1);
   }
  
  /* If the guildmaster is an alignment based class, check alignment of player vs guildmaster */
  
  if (((GET_CLASS(self) == CLASS_KNIGHT) || (GET_CLASS(self) == CLASS_CLERIC)) && IS_GOOD(self) && !IS_GOOD(ch)) {
    snprintf(buf, sizeof(buf), "%s$n tells you, %s'I cannot teach those with tainted souls.'%s", \
	CCRED(ch, C_NRM), CCNRM(ch, C_NRM), CCNRM(ch, C_NRM));
    msg = act(buf, FALSE, self, 0, ch, TO_VICT | TO_SLEEP);             
    return (1);
  }
  
  if (((GET_CLASS(self) == CLASS_KNIGHT) || (GET_CLASS(self) == CLASS_CLERIC)) && IS_EVIL(self) && !IS_EVIL(ch)) {
    snprintf(buf, sizeof(buf), "%s$n tells you, %s'I will not teach the likes of you!'%s", \
	CCRED(ch, C_NRM), CCNRM(ch, C_NRM), CCNRM(ch, C_NRM));
    msg = act(buf, FALSE, self, 0, ch, TO_VICT | TO_SLEEP);             
    return (1);
  }
  
  if (((GET_CLASS(self) == CLASS_MONK) || (GET_CLASS(self) == CLASS_DRUID)) && !IS_NEUTRAL(ch)) {
    snprintf(buf, sizeof(buf), "%s$n tells you, %s'I cannot teach you if you are not centered.'%s", \
	CCRED(ch, C_NRM), CCNRM(ch, C_NRM), CCNRM(ch, C_NRM));
    msg = act(buf, FALSE, self, 0, ch, TO_VICT | TO_SLEEP);             
    return (1);
  }
  ...
  if (skill_num < 1 ||
      GET_LEVEL(ch) < spell_info[skill_num].min_level[(int) GET_CLASS(self)]) {
	  snprintf(buf, sizeof(buf), "%s$n tells you, %s'I have no knowledge of that.'%s", \
	  CCRED(ch, C_NRM), CCNRM(ch, C_NRM), CCNRM(ch, C_NRM));
      msg = act(buf, FALSE, self, 0, ch, TO_VICT | TO_SLEEP);
    return (1);
  }
...
SPECIAL(bank)
{
  ...
  if (CMD_IS("balance")) {
    ...
    send_to_char(ch, "You deposit %s coins.\r\n", add_commas(amount));
    ...
    send_to_char(ch, "You withdraw %s coins.\r\n", add_commas(amount));
    act("$n makes a bank transaction.", TRUE, ch, 0, FALSE, TO_ROOM);
    return (TRUE);
  } else
    return (FALSE);
