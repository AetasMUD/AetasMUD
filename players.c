...
void build_player_index(void)
{
  ...
  for (i = 0; i < rec_count; i++) {
    ...
    sscanf(line, "%ld %s %d %s %ld %d %d", &player_table[i].id, arg2,
      &player_table[i].level, bits, (long *)&player_table[i].last,
	  &player_table[i].clan_name, &player_table[i].clan_rank);
...
void save_player_index(void)
{
  ...
  for (i = 0; i <= top_of_p_table; i++)
    if (*player_table[i].name) {
      ...
      fprintf(index_file, "%ld %s %d %s %ld %d %d\n", player_table[i].id,
	player_table[i].name, player_table[i].level, *bits ? bits : "0",
        (long)player_table[i].last, player_table[i].clan_name,
        player_table[i].clan_rank);
    }
  fprintf(index_file, "~\n");
...
int load_char(const char *name, struct char_data *ch)
{
  ...
  if ((id = get_ptable_by_name(name)) < 0)
	...
	else if (!strcmp(tag, "Bdge"))	GET_BADGE(ch)		= strdup(line);
	break;
	...
	else if (!strcmp(tag, "CRnk"))	GET_CLAN_RANK(ch)	= atoi(line);
	break;
	...
	else if (!strcmp(tag, "DTs "))	GET_DT_CNT(ch)	= atoi(line);
	break;
	...
	break;
      ...
      case 'G':
		 ...
		 else if (!strcmp(tag, "Gen ")) GET_GEN(ch)	= atoi(line);
	break;
	...
	case 'K':
	     if (!strcmp(tag, "Kill"))	GET_KILL_CNT(ch)	= atoi(line);
	break;
  ...
  break;
	...
	else if (!strcmp(tag, "Soul"))	GET_SOUL_POINTS(ch)	= atoi(line);
...
void save_char(struct char_data * ch)
{
  ...
  if (GET_BADGE(ch))			fprintf(fl, "Bdge: %s\n", GET_BADGE(ch));
  ...
  if (GET_REMORT(ch)   != PFDEF_REMORT)	fprintf(fl, "Rmrt: %d\n", GET_REMORT(ch));
  if (GET_GEN(ch)      != PFDEF_GEN)	fprintf(fl, "Gen : %d\n", GET_GEN(ch));
  if (GET_LEVEL(ch)	   != PFDEF_LEVEL)	fprintf(fl, "Levl: %d\n", GET_LEVEL(ch));
  if (GET_CLAN(ch)	   != PFDEF_CLAN)	fprintf(fl, "Clan: %d\n", GET_CLAN(ch));
  if (GET_CLAN_RANK(ch)	   != PFDEF_CLAN_RANK)	fprintf(fl, "CRnk: %d\n", GET_CLAN_RANK(ch));
 ...
 if (GET_SAVE(ch, 0)	   != PFDEF_SAVETHROW)	fprintf(fl, "Thr1: %d\n", GET_SAVE(ch, 0));
  ...
  if (GET_SOUL_POINTS(ch)  != PFDEF_SOUL_POINTS)	fprintf(fl, "Soul: %d\n", GET_SOUL_POINTS(ch));
 ...
 if (SCRIPT(ch)) {
  ...
  if(player_table[id].clan_name != GET_CLAN(ch)) {
      save_index = TRUE;
      player_table[id].clan_name = GET_CLAN(ch);
  }

  if(player_table[id].clan_rank != GET_CLAN_RANK(ch)) {
      save_index = TRUE;
      player_table[id].clan_rank = GET_CLAN_RANK(ch);
  }
