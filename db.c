...
void strip_car(char *string) {
  ...
  point = strchr(string, '\0');
  for (point-- ; (*point=='\r' || *point=='\n'); point--)
  	;
  *(++point) = '\0'; // always safe - we're at the position _before_ \ 0
}
...
static void interpret_espec(const char *keyword, const char *value, int i, int nr)
{
  ...
  CASE("Rmort") {
    mob_proto[i].player.remort_class = num_arg;
  }
  
  CASE("Gen") {
    mob_proto[i].player.generation = num_arg;
  }
...
void parse_mobile(FILE *mob_f, int nr)
{
  ...
  strip_car(mob_proto[i].player.long_descr);
...
char *parse_object(FILE *obj_f, int nr)
{
  ...
  char f17[READ_SIZE], f18[READ_SIZE];
  struct extra_descr_data *new_descr;
  ...
  if (((retval = sscanf(line, " %d %s %s %s %s %s %s %s %s %s %s %s %s %s %s %s %s %s %s", t, f1, f2, f3,
      f4, f5, f6, f7, f8, f9, f10, f11, f12, f13, f14, f15, f16, f17, f18)) == 4) && (bitwarning == TRUE)) {
    /* Let's make the implementor read some, before converting his world files. */
    log("WARNING: Conventional object files detected. Please see config.c.");
    exit(1);
  } else if (((retval == 17) || (retval == 3)) && (bitwarning == FALSE)) {
	...
	GET_OBJ_ANTICLASS(obj_proto + i)[0] = asciiflag_conv_aff(f13);
    GET_OBJ_ANTICLASS(obj_proto + i)[1] = 0;
    GET_OBJ_ANTIRACE(obj_proto + i)[0] = asciiflag_conv_aff(f15);
    GET_OBJ_ANTIRACE(obj_proto + i)[1] = 0;
	GET_OBJ_NEEDCLASS(obj_proto + i)[0] = asciiflag_conv_aff(f17);
    GET_OBJ_NEEDCLASS(obj_proto + i)[1] = 0;
	...
	GET_OBJ_ANTICLASS(obj_proto + i)[0] = asciiflag_conv(f13);
    GET_OBJ_ANTICLASS(obj_proto + i)[1] = asciiflag_conv(f14);
    GET_OBJ_ANTIRACE(obj_proto + i)[0] = asciiflag_conv(f15);
	GET_OBJ_ANTIRACE(obj_proto + i)[1] = asciiflag_conv(f16);
    GET_OBJ_NEEDCLASS(obj_proto + i)[0] = asciiflag_conv(f17);
	GET_OBJ_NEEDCLASS(obj_proto + i)[1] = asciiflag_conv(f18);

  } else {
    log("SYSERR: Format error in first numeric line (expecting 19 args, got %d), %s", retval, buf2);
    exit(1);
  }
...
void init_char(struct char_data *ch)
{
  ...
  // take race into account as well
  if (ch->player.sex == SEX_MALE) {
   if (ch->player.chrace == RACE_HUMAN) {
    ch->player.weight = rand_number(170, 200);
    ch->player.height = rand_number(65, 72);
   }
   if (ch->player.chrace == RACE_ELF) {
    ch->player.weight = rand_number(120, 160);
    ch->player.height = rand_number(60, 66);
   }
    if (ch->player.chrace == RACE_HALF_OGRE) {
    ch->player.weight = rand_number(300, 400); 
    ch->player.height = rand_number(84, 102);
   } 
   if (ch->player.chrace == RACE_DWARF) {
    ch->player.weight = rand_number(150, 180);
    ch->player.height = rand_number(54, 60);
   }
   if (ch->player.chrace == RACE_HALF_ORC) {
    ch->player.weight = rand_number(150, 190);
    ch->player.height = rand_number(60, 66);
   }
   if (ch->player.chrace == RACE_BAKALI) {
    ch->player.weight = rand_number(220, 250);
    ch->player.height = rand_number(72, 78);
   }
   if (ch->player.chrace == RACE_KENDER) {
    ch->player.weight = rand_number(100, 140);
    ch->player.height = rand_number(58, 64);
   }
}
  if (ch->player.sex == SEX_FEMALE) {
   if (ch->player.chrace == RACE_HUMAN) {
    ch->player.weight = rand_number(130, 170);
    ch->player.height = rand_number(63, 70);
   }
   if (ch->player.chrace == RACE_ELF) {
    ch->player.weight = rand_number(95, 130);
    ch->player.height = rand_number(58, 64);
   }
    if (ch->player.chrace == RACE_HALF_OGRE) {
    ch->player.weight = rand_number(300, 400); 
   ...
   if (ch->player.chrace == RACE_DWARF) {
    ch->player.weight = rand_number(140, 170);
    ch->player.height = rand_number(52, 58);
   }
   if (ch->player.chrace == RACE_HALF_ORC) {
    ch->player.weight = rand_number(150, 190);
    ch->player.height = rand_number(60, 66);
   ...
   if (ch->player.chrace == RACE_KENDER) {
    ch->player.weight = rand_number(100, 130);
    ch->player.height = rand_number(56, 62);
   }
 }
