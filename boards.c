...
struct board_info_type board_info[NUM_OF_BOARDS] = {
  {1206, 0, 0, LVL_GOD, LIB_ETC "board.mortal", 0},
  ...
  {1207, LVL_IMMORT, LVL_IMMORT, LVL_IMPL, LIB_ETC "board.imptodo", 0},
  {8084, 0, 0, LVL_GOD, LIB_ETC "board.cel.barb", 0},
  {8085, 0, 0, LVL_GOD, LIB_ETC "board.cel.thief", 0},
  {8086, 0, 0, LVL_GOD, LIB_ETC "board.cel.psi", 0},
  {8087, 0, 0, LVL_GOD, LIB_ETC "board.cel.monk", 0},
  {8088, 0, 0, LVL_GOD, LIB_ETC "board.cel.druid", 0},
  {8089, 0, 0, LVL_GOD, LIB_ETC "board.cel.rang", 0},
  {8090, 0, 0, LVL_GOD, LIB_ETC "board.cel.paladin", 0},
  {8091, 0, 0, LVL_GOD, LIB_ETC "board.cel.antipal", 0},
  {8092, 0, 0, LVL_GOD, LIB_ETC "board.cel.cleric", 0},
  {8093, 0, 0, LVL_GOD, LIB_ETC "board.cel.evilcleric", 0},
  {8094, 0, 0, LVL_GOD, LIB_ETC "board.cel.mage", 0},
  {8095, 0, 0, LVL_GOD, LIB_ETC "board.cel.necro", 0},
};
...
int board_write_message(int board_type, struct char_data *ch, char *arg, struct obj_data *board)
{
  ...
  snprintf(buf, sizeof(buf), "%s %-12s %s::%s %s", tmstr, buf2, CCWHT(ch, C_SPR), CCNRM(ch, C_SPR),arg);
...
int board_show_board(int board_type, struct char_data *ch, char *arg, struct obj_data *board)
{
  ...
  if (!num_of_msgs[board_type])
    ...
    len = snprintf(buf, sizeof(buf),
		...
		"%sThere are %d messages on the board.%s\r\n",
		CCCYN(ch, C_SPR), num_of_msgs[board_type], CCNRM(ch, C_SPR));
#if NEWEST_AT_TOP
    for (i = num_of_msgs[board_type] - 1; i >= 0; i--) {
      if (!MSG_HEADING(board_type, i))
        goto fubar;

      nlen = snprintf(buf + len, sizeof(buf) - len, "%s%-2d %s:%s %s\r\n", CBGRN(ch, C_SPR), num_of_msgs[board_type] - i, CBWHT(ch, C_SPR), CCNRM(ch, C_SPR), MSG_HEADING(board_type, i));
    ...
    for (i = 0; i < num_of_msgs[board_type]; i++) {
      ...
      nlen = snprintf(buf + len, sizeof(buf) - len, "%s%-2d %s:%s %s\r\n", CBGRN(ch, C_SPR), i + 1, CBWHT(ch, C_SPR), CCNRM(ch, C_SPR), MSG_HEADING(board_type, i));
      if (len + nlen >= sizeof(buf) || nlen < 0)
        break;
      len += nlen;
    }
